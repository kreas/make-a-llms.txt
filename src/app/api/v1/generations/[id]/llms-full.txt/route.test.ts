...
function req(token: string, id: string) {
  return new Request(`http://t/api/v1/generations/${id}/llms-full.txt`, { headers: { authorization: `Bearer ${token}` } });
}
...
describe('GET /api/v1/generations/[id]/llms-full.txt', () => {
  ...
  it('404 for a generation not owned by the caller', async () => {
    ...
    const [g] = await db
      .insert(generations)
      .values({ siteId: s.id, userId: other.id, status: 'succeeded', trigger: 'manual', llmsFullBlobPath: 'LF' })
      .returning();
