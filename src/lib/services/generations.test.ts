...
vi.mock('@/lib/markdown-pages/cloudflare', () => ({
  fetchPageMarkdown: vi.fn(async () => ({ markdown: '# Generated Markdown', durationMs: 10 })),
}));
...
describe('generations service tests', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  describe('readPageManifest / readPageMarkdown', () => {
    ...
    it('returns markdown for a page in the manifest when cached', async () => {
      mockGet.mockImplementation(async (path: string) => {
        if (path === 'pages/manifest.json') {
          return { stream: new Response(JSON.stringify({ pages: [{ url: 'https://example.com/a', path: 'a', blobPath: 'pages/a.md', status: 'ok', bytes: 10 }] })).body };
        }
    ...
    it('generates page markdown on the fly when the R2 blob is missing', async () => {
      mockGet.mockImplementation(async (path: string) => {
        if (path === 'pages/manifest.json') {
          return {
            stream: new Response(
              JSON.stringify({
                pages: [
                  { url: 'https://example.com/a', path: 'a', blobPath: 'pages/a.md', status: 'ok', bytes: 0 }
                ]
              })
            ).body
          };
      ...
      try {
        const { user, gen } = await seed();
        const s = await readPageMarkdown(gen.uid, user.id, 'a');
        const content = await new Response(s).text();
        
        expect(content).toContain('title: Mocked Title');
        expect(content).toContain('# Generated Markdown');
        expect(mockPut).toHaveBeenCalledWith(
          'pages/a.md',
          expect.stringContaining('# Generated Markdown'),
          expect.any(Object)
        );
      } finally {
